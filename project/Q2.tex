...
\begin{itemize}[leftmargin=1.4em, itemsep=2pt]
  ...
  \item \texttt{edge\_level[\,]}, laid out \emph{parallel to}
...
$v$ that crosses only edges with $\texttt{edge\_level}\ge k$ (whose endpoints
...
\subsection{Query time: $O\!\Big(\sum_{x\in C} d(x)\Big)$}
A query is a single DFS over the induced subgraph of the answer, scanning each
returned vertex's adjacency once. It is \textbf{output-sensitive} --- proportional
to the size of the returned component --- and bounded above by $O(n+m)$. The
initial $tc(v)<k$ rejection is $O(1)$.

\subsection{Space: $O(n+m)$}
The persistent index is $tc[\,]$ ($O(n)$) plus \texttt{edge\_level[\,]} ($O(m)$),
...
for idx in range(n):
    v = vert[idx]; level = d[v]; tc[v] = level      # smallest remaining count
    for ti in vtri[v]:                              # triangles through v
        if not alive_tri[ti]: continue
        alive_tri[ti] = 0                           # destroyed together with v
        a, b, c = triangles[ti]
        for w in (a, b, c):
            dw = d[w]
            if w == v or dw <= level: continue       # skip finalised vertices
            # move w one bucket down (O(1))
            pw = pos[w]; ps = bin_[dw]; fw = vert[ps]
            if fw != w:
                vert[pw] = fw; vert[ps] = w; pos[fw] = pw; pos[w] = ps
            bin_[dw] += 1; d[w] = dw - 1

# --- query: threshold DFS over edge levels ---
def query(self, k, v):
    if self.tc[v] < k: return []
    off, idx, lvl = self.graph.offsets, self.graph.indices, self.edge_level
    seen = {v}; stack = [v]
    while stack:
        x = stack.pop()
