...
A homomorphism only has to map every \emph{edge} of $P$ to an edge of $G$; distinct
vertices of $P$ may collapse onto the same vertex of $G$. One exists --- and here nothing
even needs to collapse:
\[
...
form a triangle and the fourth sends \emph{exactly one} edge into that triangle --- an
induced subgraph keeps \emph{every} edge of $G$ inside $S$, so two such edges would give
$5$ edges and none would leave the set disconnected.
...
$0$'' prescribes; continuing the numbering across rounds would merely shift every ID by a
constant and would change neither the entries of any $\boldsymbol{\phi}_t$ nor the kernel,
because $\boldsymbol{\phi}_t(G)$ and $\boldsymbol{\phi}_t(G')$ are always indexed the same
way.
...
Lexicographic order of the flattened tuples: $(0,0)<(0,0,0)<(0,0,0,0)$, giving
...
\]
Every vertex appears in its own two labels. That is forced rather than a stylistic
choice: to certify a direct edge $u\to v$ the shared hub must be reachable from $u$ and
must reach $v$, and in a DAG the only candidates are $u$ and $v$ themselves --- so either
$u\in L_{\mathrm{out}}(u)$ and $u\in L_{\mathrm{in}}(v)$, or $v\in L_{\mathrm{out}}(u)$ and
$v\in L_{\mathrm{in}}(v)$. Either way some vertex sits in its own label, so each hub $k$
inserts itself into $L_{\mathrm{in}}(k)$ and $L_{\mathrm{out}}(k)$ when it is processed.
(Consistently, $L_{\mathrm{out}}(k)\cap L_{\mathrm{in}}(k)=\emptyset$ before that insertion:
a hub $h\neq k$ in both would mean $k\rightsquigarrow h$ and $h\rightsquigarrow k$, a cycle.)
...
\textbf{prune}: add nothing and do not expand past $x$, since everything behind $x$ is
already covered through that same earlier hub. Otherwise add $k$ to $L_{\mathrm{out}}(x)$
...
everything behind $5$ (namely $4,8,9,10$) is likewise covered through hub $5$, so the
...
Only containment is removed: adjacent or partially overlapping intervals are kept, since
merging them would manufacture reachability that does not exist. A query is then
...
and then grows by inheriting intervals along non-tree edges. An edge $u\to w$ costs
\emph{nothing} exactly when $w$ already lies in $u$'s subtree, because then every interval
of $L(w)$ is contained in $I(u)$ and is deleted. So a spanning tree is good precisely when
it keeps mutually reachable clusters together inside one subtree.
...
\begin{align*}
  ...
  \mathrm{CONCAT}\bigl(h_B^{(0)},h_{N(B)}^{(0)}\bigr)
                  = \begin{pmatrix}0\\1\\1\\0.5\end{pmatrix}\\[4pt]
  W\cdot\begin{pmatrix}0\\1\\1\\0.5\end{pmatrix}
...
\begin{align*}
  ...
  \mathrm{CONCAT}\bigl(h_C^{(0)},h_{N(C)}^{(0)}\bigr)
                  = \begin{pmatrix}1\\1\\1\\1/3\end{pmatrix}\\[4pt]
  W\cdot\begin{pmatrix}1\\1\\1\\1/3\end{pmatrix}
...
exactly the same input vector as before. Since $\mathrm{ReLU}$ can also hide a change by
clipping it to $0$, all four vertices are computed explicitly:
...
\noindent The two recomputations:
\begin{align*}
  B:\quad h_{N(B)}^{(0)} &= \tfrac13\!\left[\begin{pmatrix}1\\0\end{pmatrix}
       +\begin{pmatrix}1\\1\end{pmatrix}+\begin{pmatrix}2\\0\end{pmatrix}\right]
       =\begin{pmatrix}4/3\\[2pt]1/3\end{pmatrix},&
  W\!\cdot\!\begin{pmatrix}0\\1\\4/3\\1/3\end{pmatrix}
    &=\begin{pmatrix}0-1+\tfrac43+0\\[2pt] 0+1-\tfrac43+\tfrac13\end{pmatrix}
     =\begin{pmatrix}1/3\\[2pt]0\end{pmatrix}\\[4pt]
  D:\quad h_{N(D)}^{(0)} &= \tfrac12\!\left[\begin{pmatrix}0\\1\end{pmatrix}
       +\begin{pmatrix}1\\1\end{pmatrix}\right]
       =\begin{pmatrix}0.5\\[2pt]1\end{pmatrix},&
  W\!\cdot\!\begin{pmatrix}2\\0\\0.5\\1\end{pmatrix}
    &=\begin{pmatrix}2-0+0.5+0\\[2pt] 0+0-0.5+1\end{pmatrix}
     =\begin{pmatrix}2.5\\[2pt]0.5\end{pmatrix}
\end{align*}
...
$h_D^{(1)}=(2.50,\,0.50)^{\mathsf T}$ (was $(3.00,\,0.00)^{\mathsf T}$).
