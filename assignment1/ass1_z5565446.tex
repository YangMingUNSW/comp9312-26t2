...
\noindent\textbf{\color{accent!85!black}Setup.} Union by size: the larger tree's root stays as
the root, and the smaller tree's root is attached under it.

\noindent\textbf{\color{accent!85!black}Tie-break.} When both trees have equal size, the tree
whose root is \emph{lexicographically smaller} is treated as larger, so its root
becomes the new root.

\noindent\textbf{\color{accent!85!black}Path compression.} Not applied.

\noindent\textbf{\color{accent!85!black}Notation.} Each node is a set element; a root is labelled
\texttt{root(size)} and its
...
The only structure that cannot enumerate neighbours in $O(\deg(v))$ is the adjacency
...
\noindent\textbf{CountCommonNeighbours query.} This is the operation that distinguishes the
...
\noindent\textbf{Assumptions and justification:}
\begin{itemize}[nosep]
  ...
  \item \textbf{Hash sets (traversals):} worst-case $O(n + m)$ --- BFS/DFS only \emph{iterate}
    each neighbour set ($O(\deg(v))$) and test the vertex-indexed \texttt{visited} array ($O(1)$);
...
\begin{itemize}[nosep]
  ...
  \item \emph{Necessary:} if some $s$--$v$ path $P$ avoids $u$, order each vertex's
    ...
    undiscovered.
\end{itemize}
\textbf{Equivalent test.} Remove $u$, then check whether $v$ is still reachable from $s$
in the graph after deleting vertex $u$.
The search below uses a queue only for this reachability check; any graph traversal
(BFS \emph{or} DFS) works equally. This is a reachability test, not a DFS simulation.
...
After the DP pass, reconstruct one optimal path by following \texttt{next} pointers.
