...
\textbf{\color{accent!85!black}Setup.} Union by size: the larger tree's root stays as
the root, and the smaller tree's root is attached under it.

\textbf{\color{accent!85!black}Tie-break.} When both trees have equal size, the tree
whose root is \emph{lexicographically smaller} is treated as larger, so its root
becomes the new root.

\textbf{\color{accent!85!black}Path compression.} Not applied.

\textbf{\color{accent!85!black}Notation.} Each node is a set element; a root is labelled
