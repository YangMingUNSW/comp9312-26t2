...
$A,B,C,D,E,F,G,H$ each of size $1$. Each drawing shows only the non-trivial trees;
any vertex not shown remains an isolated singleton.
...
+-- C
\end{verbatim}
Components: $\{B,C\}$; singletons $A,D,E,F,G,H$.
...
+-- A
\end{verbatim}
Components: $\{A,B,C\}$; singletons $D,E,F,G,H$.
...
+-- H
\end{verbatim}
Components: $\{A,B,C,D,E,F,G,H\}$ --- a single component.
...
+-- H
\end{verbatim}
Components: $\{A,B,C,D,E,F,G,H\}$ --- unchanged (still one component).
...
4. Hash-set adjacency lists & $O(\min(d_u,d_v))$ \quad $\dagger$ \\
5. CSR & $O(d_u d_v)$ \quad $\ddagger$ \\
6. Adjacency balanced BSTs & $O(\min(d_u,d_v)\,\log\max(d_u,d_v))$ \quad $\star$ \\
\bottomrule
\end{tabular}
\end{center}
